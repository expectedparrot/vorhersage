...
\usepackage{amsmath,booktabs,tabularx,array,graphicx,xcolor}
...
\input{figures/contract_rows.tex}
\input{figures/metric_rows.tex}
...
\maketitle
\begin{abstract}
We examine whether access to current primary-source evidence improves a
language model's agreement with prediction-market prices. Using the Vorhersage
forecasting workbench, we compare question-only and evidence-supplied forecasts
for six unresolved Kalshi contracts across six subject areas. The model,
instructions, and response schema are held fixed, with one independent call
per contract and condition. Opening market quotes are withheld until all
forecasts are recorded and sealed; Kalshi is excluded as a source of research
evidence. Mean absolute deviation from the opening bid--ask midpoint is
...
cases and an unchanged deviation in one. A documented entity-identification
error persists in the researched Netflix forecast. The small purposive sample,
single draw per condition, and use of prices rather than resolved outcomes
limit interpretation. The study provides a reproducible pilot of evidence
ablation and market agreement, rather than evidence of calibrated forecasting
accuracy or a general advantage from a particular forecasting workflow.
\end{abstract}

\section{Introduction}
A forecasting system must translate evidence about an event into a numerical
probability. A useful preliminary question is whether supplying current,
relevant facts changes those probabilities in a direction consistent with an
external benchmark. This study addresses that question by comparing two
...
The comparison uses contemporaneous market midpoints as reference values.
These values provide an immediate benchmark before events resolve, but they
are neither known event probabilities nor observed outcomes. We therefore
measure \emph{market agreement}, defined as proximity to a recorded quote,
and reserve claims about forecasting accuracy and calibration for evaluation
against subsequent outcomes. The design also requires that neither target
prices nor market commentary enter the forecasting inputs.
...
procedural blinding rather than independently enforced isolation: the
coordinator could technically access the evaluator database. Lexical filters
cannot exclude every indirect reference, and model pretraining exposure is
unobserved. The information supplied to the hosted calls is inspectable, but
these checks cannot establish the absence of every possible information channel.
...
additional reference. No outcome-based Brier score is computed. Neither the
bid--ask interval nor the variation across these six purposively selected
contracts is treated as a confidence interval for predictive accuracy.
...
\par\vspace{4pt}
\begin{minipage}{\linewidth}\footnotesize
\emph{Notes:} Equal-weight averages over all six contracts. The benchmark is a
quoted market price, not a resolved event outcome. The constant-50\% condition
...
\section{Discussion}
The observed pattern is consistent with current evidence improving market
agreement in this cohort. Its interpretation is narrower than a general
claim about research quality. The question-only condition lacks fresh facts
that are directly relevant to several events. The comparison therefore does
not distinguish a minimal factual update from deeper, adaptive research. Nor
does it estimate the contribution of review, scenario modeling, ensembles,
or other features of the package. Source collection was performed by an
external coding agent, so the results do not measure autonomous search by
...
interval. Third, prices may reflect liquidity and trading frictions as well
as beliefs. Neutron's opening best bid has a size of only five contracts,
which limits its interpretation as a precise reference value. Fourth,
...
identical information sets. Finally, procedural input separation does not
certify the absence of unobserved leakage or pretraining exposure.
...
uncertainty estimates that account for related event groups. Evaluation
against resolved outcomes would be required to determine whether greater
market agreement translates into better forecasts.
