...
The calling agent authors the explanation. Task authorization, factual replies, design approval and optional-review choices are distinct. Never invent a user attestation or backdate an external gate. Vorhersage requires no approval gate by default: continue authorized work. If the host requires one, reuse only authentic scoped receipts; by=user alone is not proof. Host verification/enforcement is external, and report readiness does not imply human approval. In ep-agent, load skill:report-authoring, write writeup/report.md, and follow its branding, optional-review, compilation, and checking workflow to produce writeup/report.html. This package does not call another model to narrate results. Its legacy report --project FOLDER HTML/LaTeX exports remain inspection views, not the agent's final narrative.
